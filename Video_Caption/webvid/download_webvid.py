...
def request_save(url, save_fp):
    try:
        ...
        with open(save_fp+".tmp", 'wb') as handler:
            handler.write(img_data)

        vr = ffmpeg.input(save_fp+".tmp").filter("setpts", "0.1*PTS")
        ffmpeg.output(vr, save_fp, loglevel="quiet").run(overwrite_output=True)
        os.remove(save_fp+".tmp")
    except:
        print(save_fp)
